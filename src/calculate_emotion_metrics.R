...
calculate_emotion_metrics <- function(annotations){
  ztc_mean_scores <- annotations %>%
    group_by(session, task) %>%
    group_modify(~mean_value_per_subject(.))

  ztc_trajectory_scores <- annotations %>%
    group_by(session, task) %>%
    group_modify(~trajectory_per_subject(.))

  ztc_dtw_alignedness <- annotations %>%
    group_by(session, task) %>%
